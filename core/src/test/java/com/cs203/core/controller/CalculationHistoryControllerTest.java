...
class CalculationHistoryControllerTest {
    ...
    void getCalculationsByUserId_returnsListOfCalculations() throws Exception {
        ...
        when(calculationHistoryService.getCalculationsByUserId(1L)).thenReturn(response);
    ...
    @Test
    @DisplayName("extractUserId: works when principal is Jwt")
    void saveCalculation_withJwtPrincipal() throws Exception {
        Jwt jwt = Mockito.mock(Jwt.class);
        when(jwt.getSubject()).thenReturn("99");

        Authentication jwtAuth = Mockito.mock(Authentication.class);
        ...
        SavedCalculationDto savedDto = new SavedCalculationDto();
        GenericResponse<SavedCalculationDto> response = new GenericResponse<>(
                HttpStatus.CREATED, "Calculation saved", savedDto);

        when(calculationHistoryService.saveCalculation(any(), eq(99L)))
                .thenReturn(response);
        ...
        mockMvc.perform(post("/api/v1/calculation-history")
                        ...
                        .principal(jwtAuth))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.message").value("Calculation saved"));
    }

    @Test
    @DisplayName("extractUserId throws when principal is unsupported")
    void saveCalculation_withUnsupportedPrincipal_throws() {
        Authentication auth = Mockito.mock(Authentication.class);
        when(auth.getPrincipal()).thenReturn("unsupported");
        ...
        IllegalArgumentException ex = assertThrows(IllegalArgumentException.class, () ->
                controller.saveCalculation(requestDto, auth)
        );
