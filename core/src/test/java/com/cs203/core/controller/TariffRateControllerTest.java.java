...
class TariffRateControllerTest {
    ...
    void getTariffRateById_returnsNotFound_whenTariffRateDoesNotExist() throws Exception {
        GenericResponse<TariffRateDto> response = new GenericResponse<>(HttpStatus.NOT_FOUND, "Tariff rate not found", null);
    ...
    void updateTariffRate_returnsUpdatedTariffRate() throws Exception {
        ...
        GenericResponse<TariffRateDto> response = new GenericResponse<>(HttpStatus.OK, "Tariff rate updated", updateDto);
    ...
    void post_calculate_returnsTariffCalculatorResponseDto() throws Exception {
        ...
        Mockito.when(tariffRateService.getLowestActiveTariffRate(eq(1L), eq(2L), eq(123), eq(new BigDecimal("100.00")), eq(date)))
