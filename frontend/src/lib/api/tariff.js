...
export async function calculateTariffCost(
    { hsCode, exportFrom, importTo, calculationDate, goodsValue },
) {
    try {
        const API_BASE_URL = import.meta.env.VITE_API_BASE_URL ||
            "http://localhost:8081/api/v1";
        console.log("Calculating tariff cost:", {
            hsCode,
            exportFrom,
            importTo,
            calculationDate,
    ...
    const res = await fetch(`${API_BASE_URL}/tariff-rate/calculation`, {
      ...
      body: JSON.stringify(requestBody)
        ...
        if (!res.ok) {
            ...
            if (res.status === 404) {
                throw new Error(
                    "No tariff data found for the specified countries and product",
                );
            } else if (res.status === 400) {
                throw new Error("Invalid request parameters");
            } else {
                throw new Error(
                    `Failed to calculate tariff: ${res.status} ${res.statusText}`,
                );
            }
        }
        ...
        const tariffRate = result.tariffRate
            ? parseFloat(result.tariffRate)
            : 0;
        const tariffCost = result.tariffCost
        ...
        return {
            ...
            totalCost: result.finalPrice
                ? parseFloat(result.finalPrice).toFixed(2)
                : goodsValue,
        };
    } catch (err) {
...
export async function getAllTariff() {
    try {
        ...
        if (!res.ok) {
            ...
            throw new Error(
                errorData.message ||
                    `Getting all tariffs failed: ${res.status} ${res.statusText}`,
            );
        }
...
export async function createTariff(payload) {
    try {
        ...
        if (!res.ok) {
            ...
            throw new Error(
                errorData.message ||
                    `Creating tariff failed: ${res.status} ${res.statusText}`,
            );
        }
...
export async function editTariff(payload) {
    try {
        ...
        if (!res.ok) {
            ...
            throw new Error(
                errorData.message ||
                    `Updating tariff failed: ${res.status} ${res.statusText}`,
            );
        }
...
export async function deleteSpecificTariff(id) {
    try {
        ...
        if (!res.ok) {
            ...
            throw new Error(
                errorData.message ||
                    `Deleting tariff failed: ${res.status} ${res.statusText}`,
            );
        }
